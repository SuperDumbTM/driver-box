...
<template>
  <div
    class="flex flex-col justify-center h-full max-w-full lg:max-w-xl xl:max-w-4xl mx-auto gap-y-3"
  >
    <div>返回上一頁</div>

    <form
      class="flex flex-col gap-y-3 overflow-y-auto"
      @submit.prevent="
        () => {
          if (group.drivers.length == 0) {
            $toast.warning('請至少加入一個軀動程式')
          } else {
    ...
    >
      ...
      <div>
        ...
        <div class="mb-2 max-h-[40vh] overflow-y-auto">
